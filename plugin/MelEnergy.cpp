...
{
    ALOGD("Energy initialize ...");

    if (channels != 1) {
        ALOGE("MelEnergy::initialise: channels must be 1");
        return false;
    }

    if (m_nfilters != 40) {
        ALOGE("MelEnergy::initialise: number of filters must be 40");
        return false;
    }
               ...
               UNUSED Vamp::RealTime timestamp)
{
    ...
    if (m_stepSize == 0) {
        ALOGE("MelEnergy::process: MelEnergy plugin not initialised");
